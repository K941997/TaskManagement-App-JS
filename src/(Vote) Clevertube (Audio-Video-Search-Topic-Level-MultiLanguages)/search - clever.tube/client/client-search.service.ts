...
import { SearchContentsResDto } from './dtos/res/search-contents.res.dto';
...
export class ClientSearchService {
  ...
  async searchContents(dataDto: SearchContentsReqDto, user: User) {
    ...
    const query = `(${videoQuery}) UNION (${audioQuery}) LIMIT ${SqlString.escape(
      limit,
    )} OFFSET ${SqlString.escape((page - 1) * limit)} ORDER BY id`;
